...
export default function AdminGallery() {
  ...
  return (
    <div className="p-6">
      ...
      <AddModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingItem(null); }} title={editingItem ? 'Rasmni tahrirlash' : 'Yangi rasm'} onSubmit={handleSubmit} editMode={!!editingItem}>
        <div><label className="block text-sm font-medium mb-1">Rasm *</label><div className="border-2 border-dashed p-4 text-center rounded-lg">{form.image ? <img src={form.image} className="h-32 mx-auto rounded" /> : <label className="cursor-pointer block"><i className="fas fa-cloud-upload-alt text-3xl text-gray-400"></i><p className="text-sm">Rasm yuklash</p><input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} /></label>}</div></div>
        <div><label className="block text-sm font-medium mb-1">Sarlavha (UZ)</label><input type="text" className="w-full p-2 border rounded-lg" value={form.title} onChange={e => setForm({...form, title: e.target.value})} /></div>
