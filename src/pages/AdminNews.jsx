...
export default function AdminNews() {
  ...
  const [form, setForm] = useState({ title: '', titleRu: '', content: '', image: '' });
  ...
  const handleSubmit = (e) => {
    ...
    setEditingItem(null);
    setForm({ title: '', titleRu: '', content: '', image: '' });
  };

  const handleEdit = (item) => {
    setEditingItem(item);
    setForm({ title: item.title, titleRu: item.titleRu || '', content: item.content || '', image: item.image || '' });
    setIsModalOpen(true);
  };
  ...
  return (
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={() => { setEditingItem(null); setForm({ title: '', titleRu: '', content: '', image: '' }); setIsModalOpen(true); }} className="px-4 py-2 bg-primary text-white rounded-lg hover:bg-primary/90 transition flex items-center gap-2">
      ...
      <AddModal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setEditingItem(null); }} title={editingItem ? 'Yangilikni tahrirlash' : 'Yangi yangilik'} onSubmit={handleSubmit} editMode={!!editingItem}>
        ...
        <div><label className="block text-sm font-medium mb-1">Rasm</label><div className="border-2 border-dashed p-4 text-center rounded-lg">{form.image ? <img src={form.image} className="h-24 mx-auto rounded" /> : <label className="cursor-pointer block"><i className="fas fa-cloud-upload-alt text-3xl text-gray-400"></i><p className="text-sm">Rasm yuklash</p><input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} /></label>}</div></div>
      </AddModal>
    </div>
  );
}