...
export default function Navbar() {
  ...
  const mobileMenuVariants = {
    ...
    visible: {
      x: 0,
      opacity: 1,
      transition: { type: 'spring', damping: 25, stiffness: 200, staggerChildren: 0.03 }
    },
    exit: {
      x: '100%',
      opacity: 0,
  ...
  return (
    <>
      ...
      >
        ...
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center flex-wrap gap-2 relative z-10">
          <div className="flex gap-6">
            <motion.a
              href="tel:+998653800000"
              className="flex items-center gap-2 hover:text-[#00b4d8] transition group"
      ...
      >
        <div className=" px-4">
          <div className="flex items-center justify-between">
            ...
            <Link to="/" className="flex items-center gap-3 group">
              ...
              <div>
                <motion.div
                  className="font-bold text-[#003580] text-sm md:text-base tracking-tight"
                  ...
                  <Link
                    ...
                    className={`relative px-4 py-2 rounded-xl text-sm font-medium transition-all duration-300 flex items-center gap-2 overflow-hidden ${
                      location.pathname === item.path
                        ? 'text-[#003580] bg-gradient-to-r from-[#003580]/10 to-[#0066cc]/10'
                        : 'text-gray-600 hover:text-[#003580] hover:bg-gray-50'
                    }`}
                  >
                    {/* Animated icon */}
                    <motion.i
                      className={`fas ${item.icon} text-xs`}
                      animate={{
                    ...
                    {location.pathname === item.path && (
                      <motion.div
                        className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-[#003580] to-[#0066cc] rounded-full"
                        layoutId="activeNav"
                    ...
                    {/* Hover effect */}
                    {hoveredItem === idx && location.pathname !== item.path && (
                      <motion.div
                        className="absolute inset-0 bg-gradient-to-r from-[#003580]/5 to-[#0066cc]/5 rounded-xl"
            ...
            <div className="flex items-center gap-3">
              ...
              <motion.button
                className="p-2 rounded-xl text-gray-600 hover:bg-gray-100 transition relative overflow-hidden"
                whileHover={{ scale: 1.1 }}
                whileTap={{ scale: 0.95 }}
              >
        ...
        {/* Animated underline for scrolled state */}
        {scrolled && (
          <motion.div
            className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-[#003580] to-transparent"
            initial={{ scaleX: 0 }}
            animate={{ scaleX: 1 }}
              ...
              <div className="bg-gradient-to-r from-[#003580] to-[#0066cc] p-6">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <div className="text-white font-bold text-sm">JONDOR TUMANI</div>
                      <div className="text-white/70 text-xs">{t('Rasmiy portal', 'Официальный портал')}</div>
                    </div>
                  </div>
                  <motion.button
                    onClick={() => setMobileOpen(false)}
                  ...
                  >
                    <Link
                      ...
                      className={`flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 ${
                        location.pathname === item.path
                          ? 'bg-gradient-to-r from-[#003580]/10 to-[#0066cc]/10 text-[#003580] font-semibold'
                          : 'text-gray-700 hover:bg-gray-100'
                      }`}
                    >
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                        location.pathname === item.path
                          ? 'bg-gradient-to-r from-[#003580] to-[#0066cc] text-white'
                      ...
                      {location.pathname === item.path && (
                        <motion.i
                          className="fas fa-chevron-right text-[#003580]"
                ...
                <div className="flex justify-center gap-4">
                  {['telegram', 'facebook', 'instagram', 'youtube'].map((social, i) => (
                    <motion.a
                      key={social}
                      href="#"
                      className="w-10 h-10 bg-gray-100 rounded-full flex items-center justify-center text-gray-600 hover:bg-[#003580] hover:text-white transition"
